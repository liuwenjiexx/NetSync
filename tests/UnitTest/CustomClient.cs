...
namespace UnitTest
{
    ...
    public class CustomClient : TestBase
    {
        ...
        {
            public CustomContextServer(TcpListener server)
                : base(server)
            {
            }
        ...
        class CustomContextClient : NetworkClient
        {
            ...
            private CustomContext Context { get => context; }
        ...
        {
            using (CustomContextServer server = new CustomContextServer(NewTcpListener()))
            {
                server.Start();
                CustomContextClient client = new CustomContextClient(null, NewTcpClient(), false);
                client.Start();

                client.CreateObject<CustomContext>(new StringMessage("hello"));
                foreach (var o in Wait()) yield return null;

                var serverCtx = (CustomContext)server.Objects.First();
                var clientCtx = (CustomContext)client.Connection.Objects.First();
